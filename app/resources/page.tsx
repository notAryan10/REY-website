             ...
             <div className="space-y-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {publicResources.map((res, i) => (
                   <Card key={i} accent={res.accent} className="group hover:bg-stone/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {memberResources.map((res, i) => (
                   <Card key={i} accent={res.accent} className="group hover:bg-stone/10 transition-all duration-300 relative overflow-hidden">
                      <div className="absolute inset-0 bg-background/50 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-6 text-center space-y-4">
                      ...
                      <div className="flex flex-col h-full space-y-6">
                         <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                               <h4 className="text-lg uppercase truncate">{res.title}</h4>
                               <p className="text-[10px] uppercase font-pixel tracking-widest text-text-secondary">{res.size} • {res.type}</p>
                            </div>
                         </div>
