...
export function ResourcesClient({ initialResources }: ResourcesClientProps) {
  ...
  const [resources] = React.useState<IResource[]>(initialResources);

  const userRole = session?.user?.role || "spectator";
  
  // Members are Respawner and Architect
  const isMember = status === "authenticated" && (userRole === "respawner" || userRole === "architect" || userRole === "Founder" || userRole === "Core Architect" || userRole === "Moderator");
